...
```{r}
color_palette <- c("orchid4", "darkgray")
...
assay(vsd) <- limma::removeBatchEffect(assay(vsd),batch = colData(dds)[,'batch'], design=model.matrix(~group,data=colData(dds)))
```
...
ggplot(dataGG, aes(PC1, PC2)) + 
  geom_point(size = 4, aes(color = group), show.legend = T) + 
  theme_minimal(base_size = 18) +
  xlab(paste0("PC1")) + 
  ylab(paste0("PC2")) + 
  theme(legend.position = "right")   + 
  scale_color_manual(values=color_palette, name="Stat1")+
